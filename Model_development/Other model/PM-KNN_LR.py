...
for p in [0.1]:
    ...
    for idx in range(len(test_df)):
        ...
        LR_train=X_train
        LR_y=y_train
        lr_fs = LogisticRegression(n_jobs=-1, penalty='l1')
        lr_fs.fit(LR_train, LR_y)
        col_table = pd.DataFrame({'col_name':LR_train.columns.tolist(), 'F_I':[abs(i) for i in lr_fs.coef_[0]]})
        ...
        #mat_result
        fit_train = X_train * need_wi
        fit_test = X_test * need_wi        
        ...
        if clean_idx % 10 == 0:
            ...
            continue
    group_num=int(1/p)
    test_original.iloc[:, -4:].to_csv('/home/liukang/Doc/feature_selection/{}/KNN_LR.csv'.format(group_num), index=False)